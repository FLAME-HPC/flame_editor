...
int CEMemoryModel::rowCount(const QModelIndex &/*parent*/) const
 {
     //return stringListType.count();
    return variables.count();
 }

int CEMemoryModel::columnCount(const QModelIndex &/*parent*/) const
 {
     return 3;
 }

QVariant CEMemoryModel::data(const QModelIndex &index, int role) const
 {
     if (!index.isValid())
         return QVariant();
...
QVariant CEMemoryModel::headerData(int section, Qt::Orientation orientation,
                                      int role) const
 {
     if (role != Qt::DisplayRole)
         return QVariant();
...
Qt::ItemFlags CEMemoryModel::flags(const QModelIndex &index) const
 {
     //if (!index.isValid())
...
bool CEMemoryModel::setData(const QModelIndex &index,
...
bool CEMemoryModel::insertRows(int position, int rows, const QModelIndex &/*parent*/)
 {
     ...
     for (int row = 0; row < rows; ++row)
     {
         ...
         variables.insert(position, CEVariable(s, "int"));
...
bool CEMemoryModel::removeRows(int position, int rows, const QModelIndex &/*parent*/)
 {
...
void CEMemoryModel::replaceValue(int i, double value)
{
...
void CEMemoryModel::addVariable(QString t, QString n, double i)
{
    insertRow(rowCount());
...
QStringList CEMemoryModel::getMemoryNames()
{
    QStringList list;
...
QStringList CEMemoryModel::getNames()
{
    QStringList names;
    for(int i=0; i < variables.count(); i++)
    {
