...
void Mpost::setText(QString t)
{
    ...
    for (int i = 0; i < list1.size(); ++i)
    {
        ...
        //qDebug() << "t1: " << t1;
        QStringList list2 = t1.split(" ", QString::SkipEmptyParts); // split on the spaces removing any empty elements
        qDebug() << "list2: " << list2;
        ...
        else
        {
            ...
            {
                s1.lhs = list2.at(0);
                qDebug() << "Error: lhs doesn't equal a memory variable - " << s1.lhs;
                s1.isValid = false;
            }
            ...
            if(s1.op != "=" && s1.op != "+=" && s1.op != "-=" &&
               s1.op != "/=" && s1.op != "*=")
            {
                qDebug() << "Error: op doesn't equal one of =,+=,-=,/=,*= - " << s1.op;
                s1.isValid = false;
            }
            ...
            if(s1.rhs == "")
            {
                ...
                {
                    qDebug() << "Error: rhs is not a memory value or a valid number - " << list2.at(2);
                    s1.isValid = false;
                }
            }
        }
        ...
        else qDebug() << s1.lhs << " " << s1.op << " " << s1.drhs;
        if(s1.isValid) qDebug() << "Statement is valid";
        else qDebug() << "Statement is not valid";
