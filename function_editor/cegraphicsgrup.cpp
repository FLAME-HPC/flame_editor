...
CEGraphicsGrup::CEGraphicsGrup()
{
    graphicsList = new QList<CEGraphicsItem*>;
    gap = 20;
    sub = false;
}

CEGraphicsGrup::CEGraphicsGrup(bool b)
{
    graphicsList = new QList<CEGraphicsItem*>;
    gap = 20;
    sub = true;
...
CEGraphicsGrup::CEGraphicsGrup(CEGraphicsItem *item)
{
    graphicsList = new QList<CEGraphicsItem*>;
    gap = 20;
    add(item);
...
void CEGraphicsGrup::add(CEGraphicsItem *item)
{
    qreal gapy = gap;
...
void CEGraphicsGrup::add(CEGraphicsGrup &grupLeft, CEGraphicsGrup &grupRight)
{
...
void CEGraphicsGrup::add(CEGraphicsGrup &grupRight)
{
    grups.append(&grupRight);
    positionItem.append(graphicsList->count() - 1);
}

void CEGraphicsGrup::finalArrangement()
{
    if(grups.count() > 0)
    {
        ...
        CEGraphicsGrup *grup = grups.last();
        grups.removeLast();
...
void CEGraphicsGrup::finalArrangement(QList<QRectF> &boundingList)
{
    while(grups.count() > 0)
    {
        int i = positionItem.last();
...
void CEGraphicsGrup::move(qreal x, qreal y)
{
    QPointF p(x, y);
    for(int i = 0;i < graphicsList->count();i++)
    {
        CEGraphicsItem *g = graphicsList->value(i);
        g->setPos(g->pos() + p);
    }
