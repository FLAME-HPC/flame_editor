...
int FEMemoryModel::rowCount(const QModelIndex &/*parent*/) const
 {
     //return stringListType.count();
    return variables.count();
 }

int FEMemoryModel::columnCount(const QModelIndex &/*parent*/) const
 {
     return 3;
 }

QVariant FEMemoryModel::data(const QModelIndex &index, int role) const
 {
     if (!index.isValid())
         return QVariant();
...
QVariant FEMemoryModel::headerData(int section, Qt::Orientation orientation,
                                      int role) const
 {
     if (role != Qt::DisplayRole)
         return QVariant();
...
Qt::ItemFlags FEMemoryModel::flags(const QModelIndex &index) const
 {
     //if (!index.isValid())
...
bool FEMemoryModel::setData(const QModelIndex &index,
...
bool FEMemoryModel::insertRows(int position, int rows, const QModelIndex &/*parent*/)
 {
...
bool FEMemoryModel::removeRows(int position, int rows, const QModelIndex &/*parent*/)
 {
...
void FEMemoryModel::replaceValue(int i, double value)
{
...
void FEMemoryModel::addVariable(QString t, QString n, double i)
{
    insertRow(rowCount());
...
QStringList FEMemoryModel::getNames()
{
    QStringList names;
